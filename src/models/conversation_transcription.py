...
def whisper_pyannote(text_df,speaker_df):
    ...
    for index, row in speaker_df.iterrows():
        ...
        xx_inds = ~((text_df['end'] < row_start) | (text_df['start'] > row_end))
        overlap_texts = text_df.loc[xx_inds, :]
        overlap_texts['speaker_start'] = row_start
        overlap_texts['speaker_end'] = row_end
        overlap_texts['speaker'] = speaker

        overlaps_list.append(overlap_texts)

    all_overlaps = pd.concat(overlaps_list)
    all_overlaps = all_overlaps.reset_index(drop=True)

    all_overlaps['max_start'] = np.maximum(all_overlaps['start'],all_overlaps['speaker_start'])
    all_overlaps['min_end'] = np.minimum(all_overlaps['end'],all_overlaps['speaker_end'])
    all_overlaps['overlap_duration'] = all_overlaps['min_end'] - all_overlaps['max_start']

    max_overlaps_indices = all_overlaps.groupby('id')['overlap_duration'].idxmax()

    text_speaker_df = all_overlaps.loc[max_overlaps_indices, : ]
...
def text_speaker_df_to_text(text_speaker_df): 
    ...
    for idx,this_row in text_speaker_df.iterrows():
        ...
        output_string += f'{this_start} - {this_end} : {this_speaker}\n'
        output_string += f'{this_text}\n\n'
